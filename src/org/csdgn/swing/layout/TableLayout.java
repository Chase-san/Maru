 ...
 *    distribution.
 */
package org.csdgn.swing.layout;
...
public class TableLayout implements LayoutManager, LayoutManager2 {
	private HashMap<Component,TableLayoutConstraint> defined
			= new HashMap<Component,TableLayoutConstraint>();
	...
	private Table layoutTable(Container parent) {
		...
		for(Component comp : cs) {
			...
			/*
			 * This is used to determine the number of columns and rows we
		 ...
		 * 
		 * XXX: This bit of code is a can of worms really. There is no
		 * perfect way to do it really, so I tried to imitate HTML tables
		 * as close as I care to. Unfortunately those documents say it can
		 * be done any number of ways, so WTH right?
		 */
		for(Cell cell : cells) {
